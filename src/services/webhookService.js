...
async function handleEvent(event) {
  try {
    ...
    if (!text) {
      return {
        text: "I didn't receive any text. Send 'help' for available commands.",
      };
    }
    ...
    if (user && user.connectionState) {
      console.info("User in connection flow", {
        from,
        step: user.connectionState.step,
      });
      ...
      const result = await accountConnectionService.handleConnectionInput(
        from,
        text,
      );
    ...
    if (!aiService.isConfigured()) {
      console.warn(
        "Groq API not configured, falling back to basic intent detection",
      );
...
async function executeFunctionCall(
  ...
  try {
    ...
    switch (functionName) {
        ...
        if (result.connected) {
          try {
            ...
            if (
              ...
              normalized.includes("what's my balance") ||
              normalized.includes("what is my balance")
            ) {
              const accounts = await bankService.getAccountsForUser(
                session.userId,
              );
              ...
              const resp = await aiService.generateResponseFromFunction(
                "check_balance",
                { accounts },
                conversationHistory,
                toolCallId,
            ...
            if (
              ...
              const txs = await bankService.getTransactionsForUser(
                session.userId,
                { limit: 30 },
              );
              if (!txs || txs.length === 0) {
            ...
            console.error(
              "Error auto-handling post-check_account_status intent:",
              e.message,
            );
      ...
      case "disconnect_account": {
        ...
        if (!activeAccounts.length) {
          result = {
            success: false,
            message: "You don't have any connected bank account to disconnect.",
          };
          break;
        }
        ...
        if (args && args.bank_name) {
          ...
          accountToUnlink = activeAccounts.find(
            (a) =>
              ...
              a.bankName
                .toLowerCase()
                .includes(String(args.bank_name).toLowerCase()),
          );
          ...
          result = {
            success: false,
            message:
              "You have more than one account connected. Please specify which bank to disconnect (e.g. 'disconnect Opay').",
        ...
        const unlinkResult = await monoService.unlinkAccount(
          accountToUnlink.monoAccountId,
        );
      ...
      case "check_balance":
        ...
        const isConnected =
          await accountConnectionService.isAccountConnected(phoneNumber);
        if (!isConnected) {
          return {
    ...
    return {
      text: "Sorry, I encountered an error executing that action. Please try again.",
    };
  }
}
...
async function handleTransactionConfirmation(session, text, phoneNumber) {
  ...
  if (Date.now() > pending.expiresAt) {
    ...
    return {
      text: "Transaction confirmation expired. Please try again if you still want to transfer.",
    };
  }
  ...
  if (normalized === "no" || normalized === "n" || normalized === "cancel") {
    ...
    await sessionRepo.updateSession(phoneNumber, { pendingTransaction: null });
    return {
      text: "Transfer cancelled. Is there anything else I can help you with?",
    };
  }
  ...
  switch (intent) {
    case "balance":
      ...
      const lines = accounts
        .map(
          (a) => `${validator.escape(a.name)}: ₦${a.balance.toLocaleString()}`,
        )
        .join("\n");
